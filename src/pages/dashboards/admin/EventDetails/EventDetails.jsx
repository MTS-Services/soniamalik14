...
const EventDetails = () => {
  ...
  const EventDetailsCard = ({ event }) => (
    <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-2 shadow-sm">
      ...
      <div className="text-base font-medium text-btn-primary">
        Joined: {event.joined}
      </div>
    </div>
  );
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-sm border border-gray-100 p-6 mb-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              ...
              <p className="text-base text-gray-600 mb-4">{passedEvent.type} â€¢ {passedEvent.sport} â€¢ Organized by {passedEvent.organizer}</p>
              <div className="space-y-3 text-base text-gray-700">
                ...
                <p className="text-base text-gray-600">Date: <span className="font-medium text-gray-900">{passedEvent.date}</span></p>
                <p className="text-base text-gray-600">Role: <span className="font-medium text-gray-900">{passedEvent.role}</span></p>
              </div>
            </div>
            <div className="flex flex-col items-start gap-4">
              <div className="w-full bg-gray-50 rounded-md p-4">
                <p className="text-xs text-gray-500">Status</p>
                <p className="mt-1 text-base font-medium" style={{ color: passedEvent.status === 'Approved' ? 'var(--color-btn-primary)' : 'var(--color-dashboardPending)' }}>{passedEvent.status}</p>
              </div>
              <div className="w-full">
                <button onClick={() => navigate(-1)} className="w-full px-4 py-2 rounded-md border border-gray-200 text-base">Back</button>
              </div>
            </div>
          </div>
        </div>
